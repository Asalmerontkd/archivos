	...
	{ 
	   ...
	   { 
	       ...
	       if ($aprobado == "APROBADA") {
		       ...
		       $placa = trim($datos[39]);
		       $valida="SELECT * FROM data WHERE placa = '$placa' AND fecha = '$fecha'";
		       if(mysql_num_rows(mysql_query($valida, $conn))==0)
		       {
		       		$query="INSERT INTO data VALUES('$region', '$idCentroCosto', '$centroCosto', $tarjeta, $idVehiculo, '$descVehiculo', $noComprobante, '$fecha', $codPemex, '$razonSocial', $cerCualli, '$estado', $kmAntes, $kmTransaccion, $capacidad, '$mercancia', $cantMercancia, $precioUni, $idTerminal, '$placa')";
